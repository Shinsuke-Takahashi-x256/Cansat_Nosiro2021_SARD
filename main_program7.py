...
try:
    import File_lib
    File_lib.WriteFile("All_log.txt","_start_")
except:
    pass
...
#        print(key)
        if key == "w":
            wi_motermode(1,0,0,1 , 6)
            wi_motermode(0,0,0,0 , 0.1)
        elif key == "a":
            wi_motermode(0,0,1,1 , 0.1)
            wi_motermode(0,0,0,0 , 0.1)
            wi_motermode(1,1,0,0 , 5)
            wi_motermode(0,0,0,0 , 0.1)
        elif key == "d":
            wi_motermode(1,1,0,0 , 0.1)
            wi_motermode(0,0,0,0 , 0.1)
            wi_motermode(1,1,0,0 , 5)
            wi_motermode(0,0,0,0 , 0.1)
        elif key == "s":
            ...
            wi_motermode(0,0,0,0 , 0.1)
            wi_motermode(0,0,1,1 , 0.5)
            wi_motermode(0,0,0,0 , 0.1)
            wi_motermode(1,1,0,0 , 5)
            wi_motermode(0,0,0,0 , 0.1)
        elif key == "i":
            wi_motermode(0,1,1,0 , 0.3)
            wi_motermode(0,0,0,0 , 1)
        elif key == "n":
...
def pre_main():
    ...
    if sequence_count < 3:
        ...
        C_career_cut(0)
        sequence_count = 4
        b_Moving_career_escape()
        C_career_cut(1)
        time.sleep(3)
        C_career_cut(0)

def b_Moving_career_escape():
...
def main():
    ...
    #        GPS_Rt_Array[1] = float(GPS_Rtnow_Array[1]) - temp_b
        ...
        #OR,RGを極座標変換
            print("OR",or_dis , or_rad ,"RG", rg_dis , rg_rad)
        except:
            move[0] = "goal"
            pass
        RT_rad = 0
        ...
        if move[0] == "goal":
            ...
            camera_sequence.append(move_count)
            #移動方向とゴール判定
            if out_re[0] == 0 and out_re[1] == 0 and move_count == 0:
                ETM_enabletime_move( "a" , 0.2 )
                ETM_enabletime_move( "w" , 0.1 * (camera_count ** (1/2) ) )
                t_data = ["a" , 0.2 , "w" , 0.1 * (camera_count ** (1/2) )]
                camera_sequence.append( t_data )
#                move_count = 0
            elif out_re[0] != 0 and out_re[1] != 0 and move_count < 10:
                if out_re[0] < (320 - threshold):
                    ...
                    ETM_enabletime_move( "w" , 0.5 )
                    ETM_enabletime_move( "n" , 0.08 )
                    t_data = ["w" , 0.5 , "n" , 0.08 ]
                    camera_sequence.append( t_data )
                    move_count += 1
                    print("move_count += 1")
            elif out_re[0] != 0 and out_re[1] != 0 and move_count > 9:
                print("Hello,world")
                goal_dicision = False
...
def g_inversion_check():
    if module_health["MPU_lib"] == running:
        temp = MPU_lib.Get_Accel_status()
        if temp[2] < 0:
            m_move("i",0.3)
...
def y_Moving(or_rad , rg_rad , or_dis , rg_dis , RT_rad):
    ...
    if  rg_dis <= 4e-5:
        movemode = "goal"
    elif 15e-5 < rg_dis and or_dis < 1e-5:
        movemode = "s"
    elif rad == rg_rad:
        movemode = "w"
